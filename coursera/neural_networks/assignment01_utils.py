...
def load_cat_dataset(path, file_train, file_test):
    ...
    filepath_train = os.path.join(path, file_train)
    filepath_test = os.path.join(path, file_test)

    train_dataset = h5py.File(filepath_train, "r")
    test_dataset = h5py.File(filepath_test, "r")

    return train_dataset, test_dataset


def build_cat_dataset(train_dataset, test_dataset):
...
def prepare_data(train_set_x_orig, test_set_x_orig):
    ...
    train_set_x_flatten = images2matrix(train_set_x_orig)
    test_set_x_flatten = images2matrix(test_set_x_orig)
